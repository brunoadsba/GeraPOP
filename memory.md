...
> Última atualização: 2026-08-27 (Branch `main` alinhada ao GitHub; cópia oficial de cada POP gerado em `POP - Procedimento Operacional Padrão/<CÓDIGO_NOME>/` com `.docx` + `.pdf`, no mesmo padrão das pastas inseridas pelo usuário — POP-OPE-001 Programação de Saída e POP-OPE-002 Anúncio de Navio. Histórico interno do app continua em `data/pops/`. Trabalho local ainda não commitado.)
...
- [x] Suite de testes automatizados: pytest + 9 E2E Playwright isolados
...
Estado: Branch `main` = GitHub. Saída alinhada ao padrão CODEBA (matriz 2
variantes, passos com responsável por linha, seções 7/8/9, header "Página X de Y"
página 2+, rodapé cópia não controlada, Calibri). Histórico em data/pops/;
cópia oficial em "POP - Procedimento Operacional Padrão/<CÓDIGO_NOME>/"
(docx+pdf) a cada generate/seed — preview não grava. Trabalho da pasta oficial
ainda local (não commitado).
